...
public:
	...
	template <typename T>
	IC		u32						accessible_nearest			(T const &restriction, const Fvector &position, Fvector &result, bool out_restriction);
	template <typename T>
	IC		const xr_vector<u32>	&accessible_neighbour_border(T const &restriction, bool out_restriction);
};
