...
TEMPLATE_SPECIALIZATION
IC	const typename CSolverPathManager::_edge_type &CSolverPathManager::edge		(typename _Graph::const_iterator &i) const
{
	return					((*i).m_operator_id);
}

TEMPLATE_SPECIALIZATION
IC	_dist_type CSolverPathManager::evaluate			(const _index_type &node_index1, const _index_type &node_index2, const typename _Graph::const_iterator &i) const
{
	VERIFY					(graph);
